...
func _ready():
	match type_of_rod:
		...
		"red_rod":
			rod_end_part.self_modulate = red_rod_color
			GameManager.update_rod.emit("red_rod")
		"blue_rod":
			rod_end_part.self_modulate = blue_rod_color
...
func equip_button(button_to_equip, other_button_1, other_button_2, color, rod_color):
	...
	change_rod_color_and_emit_signal(color + "_rod", rod_color)
	check_all_buttons()

func check_if_button_was_bought(button, buy_button, sprite):
